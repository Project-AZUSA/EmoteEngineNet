...
namespace EmoteEngineNet {
	namespace Adapter {
		...
		{
			typedef class IEmoteDevice_NEKO0 IEmoteDevice;
			typedef class IEmotePlayer_NEKO0 IEmotePlayer;
		private:
		...
		internal:
			EmotePlayerNEKO0(IEmotePlayer* player)
			{
		...
		public:
			...
			{
				return sPlayer->Contains(StringToCharPtr(label), x, y);
			}
		...
		ref class EmoteDeviceNEKO0 : EmoteDeviceBase
		{
			...
			typedef class IEmotePlayer_NEKO0 IEmotePlayer;
		internal:
			EmoteDeviceNEKO0(IEmoteDevice* _sEmoteDevice)
			{
		...
		public:
			EmoteDeviceBase^ EmoteCreate(IDirect3DDevice9* sD3DDevice) override
			{
				...
				IEmoteDevice* sEmoteDevice = EmoteCreate_NEKO0(initParam);
